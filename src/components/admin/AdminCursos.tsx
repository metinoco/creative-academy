  ...
  return (
    <div className={`${ADMIN_CARD} rounded-3xl border ${ADMIN_BORDER}`}>
      <div className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl font-black text-[hsl(250_60%_14%)]">
            Catálogo de cursos
          </h2>
          {!isLoading && (
            <p className="text-xs text-[hsl(250_20%_50%)] mt-0.5">
      ...
      {isLoading ? (
        <div className="p-6 space-y-3">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-center gap-3 animate-pulse">
              <div className="w-10 h-10 bg-[hsl(250_20%_90%)] rounded-lg" />
              <div className="flex-1 space-y-2">
                <div className="h-3 bg-[hsl(250_20%_90%)] rounded w-1/3" />
                <div className="h-2 bg-[hsl(250_20%_90%)] rounded w-1/5" />
              </div>
              <div className="h-6 w-20 bg-[hsl(250_20%_90%)] rounded-full" />
              <div className="h-3 w-10 bg-[hsl(250_20%_90%)] rounded" />
            </div>
          ))}
        </div>
      ) : error ? (
        <div className="p-10 text-center">
          <p className="text-sm text-[hsl(250_20%_50%)]">Error al cargar los cursos.</p>
          <button
            onClick={() => refetch()}
            className="mt-3 text-xs font-bold text-[hsl(326_85%_50%)] hover:underline"
          >
            Reintentar
          </button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-[hsl(250_30%_96%)]/60 text-[10px] uppercase tracking-widest text-[hsl(250_20%_50%)]">
              <tr>
            ...
            <tbody className={`divide-y ${ADMIN_BORDER} text-sm`}>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-sm text-[hsl(250_20%_50%)]">
                ...
                filtered.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-[hsl(250_30%_96%)]/40 transition">
                      <td className="px-6 py-3">
                        <div className="flex items-center gap-3">
                          {staticC?.image ? (
                            ...
                            <div className="w-10 h-10 rounded-lg bg-[hsl(250_20%_90%)] shrink-0" />
                          )}
                          <div className="min-w-0">
                            <div className="font-bold text-[hsl(250_60%_14%)] truncate">
                              {c.title}
                            </div>
                            <div className="text-xs text-[hsl(250_20%_50%)] truncate">{c.author}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-2 py-3">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${isPublished ? HONEY : MOSS}`}
                          />
                      ...
                      <td className="px-2 py-3 hidden md:table-cell">
                        <span className="text-xs text-[hsl(250_20%_50%)]">{c.category}</span>
                      </td>

                      <td className="px-2 py-3 text-right tabular-nums font-bold text-[hsl(250_60%_14%)]">
                        {c.active_enrollments.toLocaleString("es")}
                      </td>

                      <td className="px-2 py-3 text-right hidden sm:table-cell">
                        <span className="text-sm font-bold text-[hsl(250_60%_14%)]">{c.price}€</span>
                      </td>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Link
                                  to={`/curso/${c.slug}`}
                                  target="_blank"
                                  className="w-8 h-8 rounded-full hover:bg-[hsl(250_30%_96%)] grid place-items-center transition"
                                >
