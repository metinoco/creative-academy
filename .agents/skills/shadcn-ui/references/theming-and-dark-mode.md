...
Shadcn UI uses CSS custom properties in `globals.css` for all color tokens. Modern versions use the **oklch** color format for wider gamut support.
...
### Register with Tailwind (v3 — tailwind.config.ts)
...
> **Note:** This project uses Tailwind CSS v3. Use the `tailwind.config.ts` approach above, not the v4 `@theme` directive.
